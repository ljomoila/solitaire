...
public class Klondyke : Game
{
    ...
    public override IEnumerator Initialize()
    {
        ...
        PileSlot wasteSlot = Instantiate(
            pileSlot,
            Waste.transform.position,
            Quaternion.identity,
            Waste.transform
        );
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            pile.transform.position = new Vector3(
                pileauHolder.position.x + (xStep * i),
                pileauHolder.position.y,
                pileauHolder.position.z
            );
        ...
        for (int i = 0; i < 4; i++)
        {
            CardPile foundation = new GameObject(
                "KlondykeFoundation " + (Suit)i
            ).AddComponent<CardPile>();
            foundation.Type = PileType.Foundation;
            foundation.transform.parent = transform;
            foundation.transform.position = new Vector3(
                foundHolder.position.x + (xStep * i),
                foundHolder.position.y,
                foundHolder.position.z
            );
    ...
    public override bool TryMoveToPile(CardPile toPile, SelectionPile selectionPile = null)
    {
        ...
        if (movedCards.Count > 0 && !IsHintActive())
        {
            ...
            if (
                ...
                && sourcePile.cards.Count > 0
                && sourcePile.GetLastCard().IsTurned()
            )
            {
    ...
    private void TryTableuMove(CardPile toPile, List<Card> cards, List<Card> movedCards)
    {
        foreach (CardPile tableauPile in TableauPiles)
        {
            ...
            else
            {
                ...
                if (
                    lastPileCard.number - firstCard.number == 1
                    && Card.IsDifferentSuit(firstCard, lastPileCard)
                )
                    move = true;
            }
            if (move)
    ...
    public override IEnumerator RestoreState(XDocument xdoc)
    {
        ...
        foreach (XElement pileNode in piles.Elements("pile"))
        {
            PileType pileType = (PileType)
                System.Enum.Parse(typeof(PileType), pileNode.Element("type").Value);
            ...
            if (cards != null)
            {
                ...
                {
                    Vector3 pos = XmlHelpers.ConvertStringToVector3(
                        cardNode.Element("position").Value
                    );
                    int number = int.Parse(cardNode.Element("number").Value);
                    Suit suit = (Suit)
                        System.Enum.Parse(typeof(Suit), cardNode.Element("suit").Value);

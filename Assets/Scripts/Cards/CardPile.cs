...
public class CardPile : MonoBehaviour
{
    ...
    public virtual void AddCard(Card card, float time, float delay = 0)
    {
        ...
        iTween.MoveTo(
            ...
            iTween.Hash(
                "position",
                new Vector3(NextPos.x, NextPos.y, CardZ),
                "time",
                time,
                "delay",
                delay,
    ...
    public virtual void AlignCards(float time, float delay, int startIndex)
    {
        ...
        foreach (Card card in cards)
        {
            ...
            iTween.RotateTo(
                card.sprite.gameObject,
                iTween.Hash(
                    "rotation",
                    new Vector3(0, card.IsTurned() ? 180 : 0, 0),
                    "time",
                    animTime
                )
            );
            iTween.MoveTo(
                card.sprite.gameObject,
                iTween.Hash("position", Vector3.zero, "time", animTime, "isLocal", true)
            );
            iTween.MoveTo(
                card.gameObject,
                iTween.Hash(
                    "position",
                    new Vector3(cardX, cardY, CardZ),
                    "time",
                    animTime,
                    "isLocal",
                    true
            ...
            NextPos = card.IsTurned()
                ? new Vector2(cardX + xStepTurned, cardY - yStepTurned)
                : new Vector2(cardX + xStep, cardY - yStep);
